...
public class BinaryChromosomeGeneratorTest {
	
	@ParameterizedTest
	@MethodSource
	public void generateChromosomeTest(List<int[]> oxsList, int lengthOfOxCode, int[] begins, int[] ends, int[] result) {
		BinaryChromosomeGenerator bcg = new BinaryChromosomeGenerator();
		bcg.resetIndex();
		bcg.setLengthOfOxCode(lengthOfOxCode);
		bcg.setOxsList(oxsList);
		
		BinaryChromosome chromosome = (BinaryChromosome) bcg.generateChromosome();
